...
| `FOUNDRY_AGENT_ID` | backend | written by `setup-foundry-agent.ps1` | Created agent's ID |
| `OPS_AGENT_URL` | Foundry agent A2A connection | written by `deploy-k8s.ps1` | `https://ops-agent.<dns-zone>` |
| `OPS_AGENT_API_KEY` | A2A inbound auth | generated by `deploy-k8s.ps1`, stored in Key Vault | Pre-shared key |
